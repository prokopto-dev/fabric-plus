...
@patch('fabricplus.connection.ConnectionPlus.open_gateway', autospec=True)
@patch('paramiko.client.SSHClient.connect', autospec=True)
@patch('fabricplus.connection.ConnectionPlus.is_connected', autospec=True, return_value=True)
...
@patch('fabricplus.connection.ConnectionPlus.open_gateway', autospec=True)
@patch('paramiko.client.SSHClient.connect', autospec=True)
@pytest.mark.parametrize("connect_kwargs", [
    {"username": "username"},
    {"hostname": "hostname"},
    {"port": 22},
])
...
def test_connectionplus__client_connect_fail_timeout(self, m2):
    conn_plus = ConnectionPlus("hostname", connect_kwargs={"timeout": 10}, connect_timeout=10)
    with pytest.raises(ValueError):
        conn_plus._ConnectionPlus__client_connect(conn_plus.client)
...
@patch('fabricplus.connection.ConnectionPlus.open_gateway', autospec=True)
@patch('fabricplus.paramiko_modifications.client.SSHJumpClient.connect', return_value=None)
...
@patch('fabricplus.connection.ConnectionPlus.open_gateway', autospec=True, return_value=True)
@patch('fabricplus.paramiko_modifications.client.SSHJumpClient.connect', return_value=None)
...
@patch('fabricplus.paramiko_modifications.client.SSHJumpClient', autospec=True)
@patch('fabricplus.connection.ConnectionPlus._ConnectionPlus__client_connect', autospec=True)
...
@pytest.mark.parametrize("jump_conn", [
...
def test_connectionplus_jump_client_pass(self, m2, m3, jump_conn):
    conn_plus = ConnectionPlus("hostname", jumphost_target=jump_conn)
    assert isinstance(conn_plus.jump_client, SSHJumpClient) or isinstance(conn_plus.jump_client, SSHClient)
...
@patch('fabric.connection.Connection.open_gateway', autospec=True)
@patch('paramiko.client.SSHClient.connect', autospec=True)
@patch('fabricplus.paramiko_modifications.client.SSHJumpClient.exec_command', autospec=True, return_value=(BufferedFile(), BufferedFile(), BufferedFile()))
